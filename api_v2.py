...
def run_synthesis_task(task_id: str, text: str, original_ref_filename: str):
    ...
    try:
        ...
        # --- VITAL CHANGE: 在任务最终结束时，清空 PyTorch 的缓存 ---
        if torch.cuda.is_available():
            logger.info(f"[{task_id}] Task finished. Clearing PyTorch CUDA cache to free up memory for other services.")
            torch.cuda.empty_cache()
            logger.info(f"[{task_id}] CUDA cache cleared.")
        # -------------------------------------------------------------
